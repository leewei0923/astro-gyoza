...
export function Gallery(GalleryProps: GalleryProps) {
  ...
  return (
    <>
      <div className={`grid-cols-1 sm:grid-cols-2 auto-rows-auto grid gap-5 overflow-hidden ${currentImgs.length > 0 ? 'h-[100vh]' : ''}`}>
        {list.map((item, index) => (
          <GalleryCard
            key={`gallery${index}`}
            title={item.title}
      ...
      {currentImgs.length != 0 && (
        <section
          className={`w-full min-h-[100vh] fixed top-0 left-0 bg-black z-100 overflow-hidden z-20 ${currentImgs.length > 0 ? 'translate-y-0 h-[100vh] overflow-hidden' : '-translate-y-full'}`}
        >
